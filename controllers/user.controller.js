...
exports.createUser = async (req, res) => {
	try {
		const { email, password, role, name, phone, address, gstin } = req.body;
		if (!email || !password || !role || !name || !phone || !gstin) {
			return res.status(400).json({
				message: "Email, password, role, name, phone and gstin are required",
			});
		}
