...
const uploadFileController = async (req, res) => {
	try {
		...
		try {
			...
			return res
				.status(500)
				.json({ success: false, message: "Unable to read uploaded file" });
		}
		...
		try {
			...
			return res
				.status(400)
				.json({ success: false, message: "Invalid JSON file format" });
		}
		...
		await user.save();
		const findFile = user.files.find(
			(file) => file.filename === req.file.filename,
		);
